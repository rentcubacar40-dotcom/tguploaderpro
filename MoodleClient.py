...
class MoodleClient(object):
    ...
    def create_calendar_event_with_file(self, file_url, filename):
        ...
        try:
            ...
            # URL para crear eventos via AJAX
            event_url = f"{self.path}lib/ajax/service.php"
            
            # Preparar datos del evento
            current_date = datetime.datetime.now()
            event_name = f"Archivo: {filename}"
            event_description = f'<p>📁 <strong>{filename}</strong></p><p>🔗 <a href="{file_url}">Descargar archivo</a></p>'
            
            # Form data para el evento
            form_data = (
                f"id=0&userid={self.userid}&modulename=&instance=0&visible=1&"
                f"eventtype=user&sesskey={self.sesskey}&"
                f"_qf__core_calendar_local_event_forms_create=1&"
                f"name={quote(event_name)}&"
                f"timestart[day]={current_date.day}&"
                f"timestart[month]={current_date.month}&"
                f"timestart[year]={current_date.year}&"
                f"timestart[hour]={current_date.hour}&"
                f"timestart[minute]={current_date.minute}&"
                f"description[text]={quote(event_description)}&"
                f"description[format]=1&location=&duration=0"
            )
            ...
            resp = self.session.post(
                f"{event_url}?sesskey={self.sesskey}&info=core_calendar_submit_create_update_form",
                json=payload,
                headers=headers,
                proxies=self.proxy
            )
            ...
            result = resp.json()
            print(f"📊 Respuesta del calendario: {result}")
            
            if result and result[0].get('error') == False:
                # Éxito - retornar datos del evento
                event_id = result[0]['data']['event']['id']
                return {
                    'event_url': f"{self.path}calendar/view.php?view=day&time={int(current_date.timestamp())}",
                    'event_id': event_id,
                    'file_url': file_url
                }
            else:
                error_msg = result[0].get('exception', {}).get('message', 'Error desconocido') if result else 'Sin respuesta'
                print(f"❌ Error creando evento: {error_msg}")
                return None
                
        except Exception as e:
            print(f"❌ Error en create_calendar_event_with_file: {e}")
            return None
    ...
    def deleteFile(self, name):
        ...
        savejson = [{"index": 0, "methodname": "core_form_dynamic_form",
                     "args": {"formdata": "sesskey=" + sesskey + "&_qf__core_user_form_private_files=" + _qf__core_user_form_private_files + "&files_filemanager=" +
                                      query['itemid'] + "", "form": "core_user\\form\\private_files"}}]
        headers = {'Content-type': 'application/json', 'Accept': 'application/json, text/javascript, */*; q=0.01',
                   **self.baseheaders}
